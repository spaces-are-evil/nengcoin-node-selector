export async function getConnectedNodes() {
    ...
    const nodespromise = fetch("http://nengexplorer.mooo.com:3001/api/getpeerinfo");
    ...
    const promises = nodes.map(async element => {
        ...
        const fullCountryName = GetFullCountryName(ipdata.country);

        retdata.push({
            "ipport": element.addr,
            "countryId": ipdata.country.toLowerCase(),
...
export async function getNodeCountries(nodes) {
    ...
    nodes.filter(function (item) {
        var index = countries.findIndex(country => (country.countryId == item.countryId));
        if (index <= -1) {
...
export async function getCurrentBlock() {
    const currentblock = fetch("http://nengexplorer.mooo.com:3001/api/getblockcount")
